...
export class AppService {
  calcRepaymentPlan(
    ...
    const months = calcMonthlyRepaymentPlan(
      loanAmount,
      interestRate,
      initialRepayment,
    );
...
const transformMonthsIntoYears = (
  months: RepaymentPlanEntry[],
): RepaymentPlanEntry[] => {
  const chunks = chunk(months, 12);

  const years = chunks.map((chunk, chunkIndex) => {
    return chunk.reduce(
      (acc: RepaymentPlanEntry, curr: RepaymentPlanEntry) => {
        const rate = fixedNumber(acc.rate + curr.rate);
        const interestPortion = fixedNumber(
          acc.interestPortion + curr.interestPortion,
        );
        const repaymentShare = fixedNumber(
          acc.rePaymentShare + curr.rePaymentShare,
        );
...
const calcMonthlyRepaymentPlan = (
  loanAmount: number,
  interestRate: number,
  initialRepayment: number,
): RepaymentPlanEntry[] => {
